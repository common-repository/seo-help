...
if( !class_exists('QcldSeoHelpPluginUpgradeToProNotice') )
{
	class QcldSeoHelpPluginUpgradeToProNotice {
		...
		{
			add_action('admin_head', array(&$this, 'qcld_seohelp_pro_upgrade_to_pro_heading_part'), 100 );
		}

		function qcld_seohelp_pro_upgrade_to_pro_heading_part()
		{
			if( is_admin() ){
			?>
				<script>
		 ...
		 *******************************/
		function qcld_seohelp_pro_check_if_plugin_page()
		{
		 ...
		 *******************************/
		function qcld_seohelp_pro_hook_with_plugin_action_links(){
		...
		function func_show_upgrade_link_with_action_links( $links )
		{
			$links = array_merge( $links, array(
				'<a title="'.$this->link_text.'" class="'.$this->link_class.'" style="background:#0000ff; padding: 2px 5px; font-weight: bold; color: '.$this->link_color.';" href="' . esc_url( $this->upgrade_link ) . '" target="'.$this->link_target.'">' . __( $this->link_text, 'quantumcloud' ) . '</a>'
			) );
		 ...
		 * Ofcourse, in plugin.php page
		 *******************************/
		function qcld_seohelp_pro_hook_with_plugin_meta_links(){
		...
		function func_show_upgrade_link_with_meta_links( $links, $file )
		{
			...
			if ( strpos( $file, "$this->plugin_main_file" ) !== false ) {
				...
				$new_links = array(
					'<a class="'.$this->link_class.'" style="background:#0000ff; padding: 2px 5px; font-weight: bold; color: '.$this->link_color.';" href="' . esc_url( $this->upgrade_link ) . '" title="'.$this->link_text.'" target="'.$this->link_target.'">' . __( $this->link_text, 'quantumcloud' ) . '</a>'
				);
		 ...
		 *******************************/
		function qcld_seohelp_pro_hook_with_plugin_submenu()
		{
		   	if( !$this->show_with_plugin_menu || $this->plugin_menu_slug == "" ){
		     	return;
		   	}else{
			  	add_action( 'admin_menu' , array(&$this, 'qcld_seohelp_pro_func_qc_external_upgrade_link'), 100 );
		   	}
		 ...
		 *******************************/
		function qcld_seohelp_pro_func_qc_external_upgrade_link()
		{
		    global $submenu;
		    ...
		    $link_text = '<span class="qc-up-pro-link" style="font-weight: bold; padding: 5px; background: #FCB214; border-radius: 4px; color: #0000ff">'.$this->link_text.'</span>';
...
$instance_sldf2 = new QcldSeoHelpPluginUpgradeToProNotice();
...
{ 
	...
	$instance_sldf2->upgrade_link = "https://www.dna88.com/product/seo-help-pro/";

	$instance_sldf2->plugin_slug = 'seo-help'; //Plugin Slug. i.e. Folder Name
	$instance_sldf2->plugin_main_file = 'qc-project-linkbait-main.php'; //Primary file of the pluign
	$instance_sldf2->plugin_menu_slug = 'qcld-seo-help'; //Main Menu Slug
	...
	if( $instance_sldf2->qcld_seohelp_pro_check_if_plugin_page() )
	{
		$instance_sldf2->qcld_seohelp_pro_hook_with_plugin_action_links();
		$instance_sldf2->qcld_seohelp_pro_hook_with_plugin_meta_links();
	}
	
	$instance_sldf2->qcld_seohelp_pro_hook_with_plugin_submenu();
