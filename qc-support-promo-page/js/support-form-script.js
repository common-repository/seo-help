(function($) {
	...
	jQuery(document).ready(function($){
	  	...
	  	$(document).on("click","#qcpg-query-submit-btn", function(event){
			...
			if( flag )
			{
				...
				var data = {
					...
					'security': qcld_seo_recommend_ajax_nonce
				};

				jQuery.post(qcld_seo_recommend_ajaxurl, data, function(response) {
		...
		$( window ).on( "load", function() {
			...
			var data = {
				'action': 'qcld_seo_recommend_support_function_ajax',
				'security': qcld_seo_recommend_ajax_nonce
			};

	        jQuery.post(qcld_seo_recommend_ajaxurl, data, function (response) {

	           	$('.qcld-seo-support').find('.qcld-plugins-lists').html(response);
