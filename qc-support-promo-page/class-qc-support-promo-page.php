...
if ( ! defined( 'qcld_seo_support_path' ) ) {
    define('qcld_seo_support_path', plugin_dir_path(__FILE__));
}

if ( ! defined( 'qcld_seo_support_url' ) )
    define('qcld_seo_support_url', plugin_dir_url( __FILE__ ) );

if ( ! defined( 'qcld_seo_support_img_url' ) )
    define('qcld_seo_support_img_url', qcld_seo_support_url . "/images" );
...
if ( ! function_exists( 'qcld_seo_include_promo_page_scripts' ) ) {	
	function qcld_seo_include_promo_page_scripts( ) {   


        if( isset($_GET["page"]) && !empty($_GET["page"]) && (   $_GET["page"] == "qcld-seo-help-supports"  ) ){

            wp_enqueue_style( 'qcld-support-fontawesome-css', qcld_seo_support_url . "css/font-awesome.min.css");                              
            wp_enqueue_style( 'qcld-support-style-css', qcld_seo_support_url . "css/style.css");

            wp_enqueue_script( 'jquery' );
            wp_enqueue_script( 'jquery-ui-core');
            wp_enqueue_script( 'jquery-ui-tabs' );
            wp_enqueue_script( 'jquery-qcld_seo-form-processor', qcld_seo_support_url . 'js/support-form-script.js',  array('jquery', 'jquery-ui-core','jquery-ui-tabs') );

            wp_add_inline_script( 'jquery-qcld_seo-form-processor', 
                                    'var qcld_seo_recommend_ajaxurl    = "' . admin_url('admin-ajax.php') . '";
                                    var qcld_seo_recommend_ajax_nonce  = "'. wp_create_nonce( 'qc-clr' ).'";   
                                ', 'before');
            
        }
	   
	}
	add_action('admin_enqueue_scripts', 'qcld_seo_include_promo_page_scripts');
...
include_once qcld_seo_support_path . '/qc-clr-recommendbot-support-plugin.php';

if ( ! function_exists( 'qcld_seo_promo_support_page_callback_func' ) ) {

	function qcld_seo_promo_support_page_callback_func() {
		
?>


        <div class="qcld-seo-support qcld-support-new-page">
            ...
            <div class="qcld-plugins-lists">
                <div class="qcld-plugins-loading">
                    <img src="<?php echo esc_url(qcld_seo_support_img_url); ?>/loading.gif" alt="loading">
                </div>
            </div>
        </div>
 ...
 *******************************/
add_action( 'wp_ajax_qcld_seo_process_qc_promo_form', 'qcld_seo_process_qc_promo_form' );

if( !function_exists('qcld_seo_process_qc_promo_form') ){

    function qcld_seo_process_qc_promo_form(){
